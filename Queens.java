...
public class Queens {
  ...
  public static void simulate(int generations, int size, int dimension) {
    ...
    Population solutions = new Population (size , dimension);
    Individual best = solutions.getFittest();
    int c = 0;
    
    
    A:for(int i = 0 ; i < generations ; i++){
      ...
      if (best.getFitness() == 0)
        ...
        solutions.evolve();
        best = solutions.getFittest();
        c++;
      }
    }    

       System.out.println("Generations: " + c+", Individual: (fitness="+best.getFitness() +", "+ best.toString());
       String infoMessage = ("Generations: " + c+", Individual: (fitness="+best.getFitness() +", "+ best.toString());
    GUI check = new GUI (best.getSize());
    check.Graphics(best);
            JOptionPane.showMessageDialog(null, infoMessage,"Assignment 1 by Isfandyar Virani and Riyanson Alfred", JOptionPane.INFORMATION_MESSAGE);
